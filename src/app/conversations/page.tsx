...
import * as THREE from 'three';
import { Mic } from 'lucide-react';
// Import GLTFLoader correctly for client-side
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import MicrophoneComponent from '../test/page';
// This component handles the Three.js scene
export default function ThreeJSScene() {
  const canvasRef = useRef(null);
  const sceneRef = useRef(null);
    ...
    const loadModel = (scene) => {
      ...
      textureLoader.load(
        ...
        (texture) => {
          ...
          // Now load the model with the GLTFLoader
          const loader = new GLTFLoader();
          loader.load(
            'https://s3-us-west-2.amazonaws.com/s.cdpn.io/1376484/stacy_lightweight.glb',
            (gltf) => {
      ...
      {/* <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 w-full max-w-md">
        <div className="bg-white bg-opacity-80 rounded-lg p-4 shadow-lg">
          <form onSubmit={handleSendMessage} className="flex items-center space-x-4">
            <button 
              type="button" 
              className={`p-2 rounded-full text-white ${isRecording ? 'bg-red-500' : 'bg-blue-500'}`}
        ...
        <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 w-full max-w-md">
          <MicrophoneComponent />
        </div>
    </div>
    </>
